      ...
      <div className="docs-content" ref={contentRef}>
        ...
        <DocSection id="alerts" title="Alerts">
          <p>
            ...
            backend scheduler. Alert rules are managed through the <strong>Settings</strong> page.
          </p>
          <div className="docs-sub">Severity levels</div>
          <div className="docs-status-row"><span className="docs-status-dot" style={{ background: "var(--clr-red)" }} /> <strong>Error</strong> — critical condition (e.g., cluster health red, heap above 95%)</div>
          ...
          <div className="docs-sub">Alert rule types</div>
          <ul>
            <li><strong>Disk Usage</strong> — fires when cluster disk usage exceeds a percentage threshold (default 80%)</li>
            <li><strong>ILM Error</strong> — fires when any index has a failed ILM step or error</li>
            <li><strong>Ingest Stall</strong> — fires when an index matching a pattern receives no new documents for N minutes</li>
            <li><strong>Cluster Health Change</strong> — fires when cluster status degrades (e.g., green to yellow, yellow to red)</li>
            <li><strong>Heap Pressure</strong> — fires when any node's JVM heap usage exceeds a threshold (default 85%)</li>
            <li><strong>Unassigned Shards</strong> — fires when unassigned shards persist for longer than N minutes (default 10)</li>
            <li><strong>Shard Count</strong> — fires when total shard count exceeds a limit (default 1000)</li>
          </ul>
          <div className="docs-sub">How alerts work</div>
          <p>
            The backend runs all enabled alert rules every 5 minutes against every configured cluster.
            When a rule fires, the alert is stored in a local SQLite database and optionally sent
            to a <strong>Slack webhook</strong> if configured.
          </p>
          <div className="docs-tip">
            <strong>Tip:</strong> Manage alert rules and Slack webhook from the Settings page (gear icon
            in the icon rail). Changes take effect immediately — no restart required.
          </div>
        </DocSection>
        ...
        <DocSection id="settings" title="Settings">
          <p>
            The <strong>Settings</strong> page lets you manage alert rules and Slack integration
            through the UI. Access it via the gear icon at the bottom of the icon rail.
          </p>
          <div className="docs-sub">Slack integration</div>
          <p>
            Enter your Slack webhook URL to receive alert notifications. The URL is stored securely
            and displayed masked (only the last 8 characters are visible). Click <strong>Save</strong> to
            update the webhook.
          </p>
          <div className="docs-sub">Managing rules</div>
          <ul>
            <li><strong>Add a rule</strong> — click "+ Add alert rule", select a type, configure thresholds, and save</li>
            <li><strong>Edit a rule</strong> — click the pencil icon on any rule card to modify its name, type, or config</li>
            <li><strong>Enable/disable</strong> — use the toggle switch. Disabled rules are dimmed and skipped during checks</li>
            <li><strong>Delete a rule</strong> — click the X icon. You'll be asked to confirm before deletion</li>
          </ul>
          <p>
            All changes take effect immediately — the alert scheduler restarts with the new configuration
            as soon as you save, toggle, or delete a rule.
          </p>
          <div className="docs-sub">Migration from config.yml</div>
          <p>
            On first startup, Elkwatch automatically imports any alert rules defined in <code>config.yml</code>
            into the database. After that, the database is the sole source of truth — alert rules in
            config.yml are ignored. Cluster configuration still lives in config.yml.
          </p>
        </DocSection>
        ...
        <DocSection id="config" title="Configuration">
          ...
          <p>
            Alert rules are managed through the <strong>Settings</strong> page (gear icon in the icon rail).
            On first startup, rules from <code>config.yml</code> are automatically imported into the
            database. After that, use the Settings UI to add, edit, or remove rules.
          </p>
